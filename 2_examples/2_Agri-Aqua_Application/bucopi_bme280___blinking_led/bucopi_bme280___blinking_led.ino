...
void setup() {
  ...
  Wire.begin(); // No arguments for specifying SDA and SCL
  pinMode(LED_PIN, OUTPUT);
  digitalWrite(LED_PIN, LOW); // Ensure LED is initially off

  // default settings
  status = bme.begin(0x76);  // 0x76 is the default I2C address for BMP280
  if (!status) {
    Serial.println("Could not find a valid BME280 sensor, check wiring!");
    while (1);
  }
  Serial.println("Sensor is Good to Go");
...
void handleGetData() {
  // Read sensor data
  float temperature1 = readBME280Temperature();
  float humidity1 = readBME280Humidity();
  float pressure1 = readBME280Pressure();

  // Create a JSON document to hold the sensor data
  StaticJsonDocument<200> jsonData;
  jsonData["temperature"] = temperature1;
  jsonData["humidity"] = humidity1;
  jsonData["pressure"] = pressure1;
...
void BME_Get_Data() {
  ...
  delay(5); // Delay for 1 second before taking another reading
}
...
void handleRoot() {
  ...
  html += ".container {max-width: 400px; margin: 0 auto; padding: 20px; background-color: #fff; border: 1px solid #ccc; border-radius: 5px; box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.2);}";
  ...
  html += "<p><strong>Temperature:</strong> <span id='temperature'>Loading... </span> C</p>";
  html += "<p><strong>Humidity:</strong> <span id='humidity'>Loading... </span> %</p>";
  html += "<p><strong>Pressure:</strong> <span id='pressure'>Loading... </span> hPa</p>";


  // Add a button to control the relay
  html += "<p><button id='ledButton' onclick='toggleLED()'>" + (ledState ? String("Turn OFF LED") : String("Turn ON LED")) + "</button></p>";
  html += "</div>";
  html += "</body>";
  html += "<script>";
  // html += "console.log('hello')";
  // Function to update sensor data using AJAX
  ...
  html += "document.getElementById('temperature').textContent = data.temperature.toFixed(2);";
  html += "document.getElementById('humidity').textContent = data.humidity.toFixed(2);";
  html += "document.getElementById('pressure').textContent = data.pressure.toFixed(2);";
  html += "}";
  html += "};";
  html += "xhr.send();";
  html += "}";

  // Periodically update sensor data (every 5 seconds in this example)
  ...
  html += "      updateLEDButton(data);";
  html += "    });";
  html += "}";
  html += "function updateLEDButton(status) {";
  html += "  var button = document.getElementById('ledButton');";
  html += "  if (status === 'LED ON') {";
  html += "    button.innerHTML = 'Turn OFF LED';";
  html += "  } else {";
  html += "    button.innerHTML = 'Turn ON LED';";
  html += "  }";
  html += "}";
